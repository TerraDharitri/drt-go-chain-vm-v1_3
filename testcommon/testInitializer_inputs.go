...
// DCDTTestTokenKey is an exposed value to use in tests
var DCDTTestTokenKey = worldmock.MakeTokenKey(DCDTTestTokenName, 0)
...
// BuildSCModule invokes drtpy to build the contract into a WASM module
func BuildSCModule(scName string, prefixToTestSCs string) {
	pathToSCDir := prefixToTestSCs + "test/contracts/" + scName
	out, err := exec.Command("drtpy", "contract", "build", "--no-optimization", pathToSCDir).Output()
	if err != nil {
